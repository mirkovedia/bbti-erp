...
async function main() {
  try {
    const logUser = await db.users.findUnique({ where: { email: 'logistica@bbti.com.pe' }, select: { id: true } });
    const countLog = async () => await db.notificaciones.count({ where: { destinatario_id: logUser.id } });
    ...
    const id = proy.id;
    const ingUser = await db.users.findUnique({ where: { email: 'ingenieria@bbti.com.pe' }, select: { id: true } });
    const hitoCount = await db.notificaciones.count({ where: { proyecto_id: id, destinatario_id: ingUser.id, tipo: 'hito' } });
    ...
    await db.proyecto_documentos.create({ data: { proyecto_id: id, nombre: 'plano.pdf', estado: 'Aprobados y firmados' } });
