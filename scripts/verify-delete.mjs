...
async function main() {
  ...
  try {
    ...
    const cookieRaw = await getAuthCookie('admin@bbti.com.pe', 'admin2024');
    proy = await (await fetch(`${BASE}/api/proyectos`, { method: 'POST', headers: { 'Content-Type': 'application/json', cookie: cookieRaw }, body: JSON.stringify({ cliente: 'PARA BORRAR UI', monto: 50 }) })).json();
    ...
    const data = await db.proyectos.findMany({ where: { id: proy.id }, select: { id: true, activo: true } });
    ...
    if (proy && proy.id) {
      await db.proyectos.deleteMany({ where: { id: proy.id } });
      await db.actividad_log.deleteMany({ where: { proyecto_id: proy.id } });
    }
    await browser.close();
    await db.$disconnect();
    console.log(`\n===== ${pass} OK / ${fail} fallos =====`);
