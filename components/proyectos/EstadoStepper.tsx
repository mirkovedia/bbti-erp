...
export const EstadoStepper = ({ proyecto }: Props) => {
  ...
  return (
    <div className="w-full bg-[var(--navy2)] rounded-xl border border-slate-800 overflow-x-auto scrollbar-none">
      <div className="flex items-center min-w-[580px] md:min-w-0 md:w-full justify-between px-6 py-4">
          ...
          return (
            <Fragment key={stage}>
              ...
              {i < FLOW_STAGES.length - 1 && (
                <div className={cn('flex-1 h-0.5 mx-2 -mt-5 transition-colors min-w-[30px]', isDone ? 'bg-green-500' : 'bg-slate-700')} />
              )}
            </Fragment>
          );
        })}
